...
### Регистрация
[URL регистрация](https://movie-hub.ru/registration).
...
- Все слова в полях ввода размером 19px. Подписи над полями ввода размером 13px. Заголовок блока размером 20px c жирным шрифтом.
...
### Авторизация
[URL авторизация](https://movie-hub.ru/login).
...
- Размер полей ввода для логина и пароля равняется 360p * 56p.
...
## Настройки
[URL настроек](https://movie-hub.ru/settings). Для данный страницы необходима авторизация.
<a id="setup"></a>

По-умолчанию запрещено изменять данные профиля, необходимо нажать на кнопку редактирования.</br>
![alt text](img/settingNoEdit.png)</br>
Все слова в полях ввода размером 19px. Подписи над полями ввода размером 13px. Заголовок блока размером 20px c жирным шрифтом.


1. Аватарка
    - У нового пользователя отображается аватарка по умолчанию.</br>
      ![alt text](img/settingsDefault.png)
    - При нажатии на кнопку изменения аватарки появляется системный интерфейс выбора файла для загрузки на сайт. Размер 308p * 55p, с цветом #333333.</br>
      ![alt text](img/settingChange.png)
    - Предлагаемые расширения загружаемых файлов jpg, jpeg, png, webp.
    - Загруженное изображение сразу отображается.
    - Изменение аватарки применяется после нажатия кнопки "сохранить". Размер 308p * 38p, с цветом #333333.</br>
      ![alt text](img/settingSave.png)

2. Дата рождения
    - Дата рождения соответствует дате, указанной при регистрации. Размер поля ввода равен 342p * 71px.
...
3. Email
    - При пустом поле появляется ошибка. Размер поля ввода равен 342p * 71px.
    - При заполнении поля невалидной почтой (без символа @) появляется ошибка.
    - __BUG:__ При заполнении кириллицей появляется ошибка (хотя есть адреса и домены с кириллическими символами).
    - При наличии в базе введенной почты выдается ошибка в том, что такая почта уже существует.

4. Пароль и подтверждение пароля
    - При заполнении валидным паролем (латиницей длиннее 8 символов и содержащим хотя бы 1 цифру и 1 букву) изменение пароля проходит успешно. Размер полей ввода равен 342p * 71px.
...
[URL расширенного поиска](https://movie-hub.ru/userStatistic). Для данный страницы необходима авторизация.
<a id="stats"></a>

- После написания отзыва на фильм, в раскрывающемся списке “Мой профиль” в навигационного меню, в панели [Статистика](https://movie-hub.ru/userStatistic), появляются столбики гистограммы с соответствующими текущему фильму жанрами. Размер столбцов гистограммы зависит от среднего рейтинга и потому варируется от 0p * 20p до 320p * 20p.</br>
![alt text](img/stat-image-2.png)
- Все слова в гистограмме размером 13px. Заголовок блока размером 25px.
- В случае отсутствия отзывов на фильмы, в раскрывающемся списке “Мой профиль” в навигационного меню, в панели [Статистика](https://movie-hub.ru/userStatistic), будут отсутствовать гистрограммы по жанрам.</br>
м
...
- Всего количество жанров равняется 15: Боевики, военные, детективы, драммы, коммедии, криминальные, аниме, дорама, мультфильмы, детские, ужасы, мелодрамы, фантастика, триллер, фэнтези.
- __BUG:__ пункт Последние фильмы” отображает посещенные страницы фильмов справа-налево.
- Размеры постеров фильмов равны 200p * 300p. Боковые кнопки прокручивания слайдеров равны 50p * 50p.</br>
![alt text](img/statSlider.png)

## Расширенный поиск по фильмам и актерам
[URL расширенного поиска](https://movie-hub.ru/selection).
- Все слова в окне поиска актера/фильма размером 19px. Заголовок блока размером 32px. Подзаголовок размером 22px. Размер шрифта с полями ввода равна 19px.
...
1. Поиск по фильмам
    ...
    - При нажатии на кнопку поиска без настройки фильтров выдаются все существующие фильмы. Кнопка поиска размером 207p * 38p, цветом #f5c518.
    - Поля ввода названия фильма, актеров размером 480p * 71p. Фон белый.
    - Поля рейтинга размером по 210p * 71p. Фон белый.
    - Поля рейтинга размером по 165p * 71p. Фон белый.
...
2. Поиск по актерам
    - При нажатии кнопки “Показать результат” выдаются все актеры/фильмы. Кнопка поиска размером 207p * 38p, цветом #f5c518.
    - Все поля ввода размером 480p * 71p. Фон белый.
...
## Календарь
[URL начальной страницы](https://movie-hub.ru/). Календарь в самом низу начальной страницы.
...
- При нажатии на ячейку с назначенным релизом фильма нас редиректит на страницу фильма. Размер ячейки составляет 144p * 144p.</br>
![alt text](img/calendar-release.jpg)
- Все цифры в календаря размером 19px. Заголовок блока размером 25px. Подзаголовок размером 16px. Размер шрифта с названиями фильмов рамен 16 px.
- При нажатии на колокольчик (подписка на уведомления о выходе нового фильма) в авторизованном режиме появляется табличка с предложением разрешить принимать уведомления. Размер колокольчика составляет 50p * 50p.
